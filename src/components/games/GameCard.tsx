...
export function GameCard({ game, locale }: GameCardProps) {
  ...
  const requiresLogin = game.isLive;

  return (
    <Link
      href={href}
      className="group block relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 hover:border-red-700/60 hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-red-900"
    >
      <div className="relative aspect-video bg-black overflow-hidden">
        ...
        {game.isFeatured && (
          <span className="absolute top-2 left-2 bg-red-900 text-red-200 text-xs font-mono px-2 py-0.5 rounded uppercase tracking-wider">
            <span className="inline-flex items-center gap-1"><Sparkles className="h-3 w-3" /> {t('badge_featured')}</span>
          </span>
        )}
        {game.isMultiplayer && (
        ...
        {requiresLogin && (
          <span className="absolute bottom-2 left-2 z-[2] bg-zinc-900/85 text-zinc-100 text-[10px] font-mono px-2 py-1 rounded uppercase tracking-wider inline-flex items-center gap-1">
            <Lock className="h-3 w-3" /> Login required
          </span>
        )}
      </div>
      <div className="p-4">
        <h3 className="font-mono text-white font-bold tracking-widest text-sm uppercase mb-1 group-hover:text-red-300 transition-colors inline-flex items-center gap-2">
