// ═══════════════════════════════════════════════════════════════════════════
// Web Worker: Hash Generator
// Offloads cryptographic hash computation to a background thread
// ═══════════════════════════════════════════════════════════════════════════

interface HashRequest {
  text: string;
  algorithms: string[]; // e.g. ['MD5', 'SHA-1', 'SHA-256', 'SHA-512']
}
...
async function computeHash(text: string, algorithm: string): Promise<string> {
  ...
  // Map friendly names to Web Crypto API algorithm names
...
// Simple MD5 implementation for the worker
...
self.onmessage = async (e: MessageEvent<HashRequest>) => {
  ...
  try {
    ...
    const response: HashResponse = { success: true, results };
